...
@section('title', __('products.register_new'))
...
<div class="dashboard-container">
    <div class="page-header">
        <div class="page-header-content">
            ...
            <div>
                <h1>{{ __('products.register_new') }}</h1>
                <p>{{ __('products.register_new_desc') }}</p>
            </div>
        </div>
    </div>

    <form action="{{ route('representative.pharmaceutical-products.store') }}" method="POST" class="product-form">
        @csrf
        ...
        <div class="form-section">
            ...
            <div class="form-row">
                <div class="form-group">
                    ...
                    <input type="text" id="pharmaceutical_form" name="pharmaceutical_form" class="form-control @error('pharmaceutical_form') is-invalid @enderror" value="{{ old('pharmaceutical_form') }}" placeholder="{{ __('products.dosage_form_example') }}" required>
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="concentration" name="concentration" class="form-control @error('concentration') is-invalid @enderror" value="{{ old('concentration') }}" placeholder="{{ __('products.concentration_example') }}" required>
                    @error('concentration')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="form-group">
                ...
                <div class="checkboxes-group">
                    <label class="checkbox-label">
                        <input type="checkbox" name="usage_methods[]" value="oral" {{ in_array('oral', old('usage_methods', [])) ? 'checked' : '' }}>
                        <span>{{ __('products.usage_oral') }}</span>
                    </label>
                    <label class="checkbox-label">
                        <input type="checkbox" name="usage_methods[]" value="injection" {{ in_array('injection', old('usage_methods', [])) ? 'checked' : '' }}>
                        <span>{{ __('products.usage_injection') }}</span>
                    </label>
                    <label class="checkbox-label">
                        <input type="checkbox" name="usage_methods[]" value="topical" {{ in_array('topical', old('usage_methods', [])) ? 'checked' : '' }}>
                        <span>{{ __('products.usage_topical') }}</span>
                    </label>
                    <label class="checkbox-label">
                        <input type="checkbox" name="usage_methods[]" value="inhalation" {{ in_array('inhalation', old('usage_methods', [])) ? 'checked' : '' }}>
                        <span>{{ __('products.usage_inhalation') }}</span>
                    </label>
                    <label class="checkbox-label">
                        <input type="checkbox" name="usage_methods[]" value="other" id="usage_other" {{ in_array('other', old('usage_methods', [])) ? 'checked' : '' }}>
            ...
            <div class="form-group" id="other_usage_method_group" style="display: {{ in_array('other', old('usage_methods', [])) ? 'block' : 'none' }};">
                <label for="other_usage_method">{{ __('products.specify_usage') }}</label>
                <input type="text" id="other_usage_method" name="other_usage_method" class="form-control @error('other_usage_method') is-invalid @enderror" value="{{ old('other_usage_method') }}" placeholder="{{ __('products.type_usage') }}">
        ...
        <div class="form-section">
                ...
                <select id="foreign_company_id" name="foreign_company_id" class="form-control @error('foreign_company_id') is-invalid @enderror" required>
                    ...
                    @foreach($foreignCompanies as $company)
                        <option value="{{ $company->id }}"
                            ...
                            {{ old('foreign_company_id') == $company->id ? 'selected' : '' }}>
            ...
            <div id="local_company_info" class="local-company-info" style="display: {{ old('foreign_company_id') ? 'block' : 'none' }};">
                <div class="info-card">
        ...
        <div class="form-actions">
            <a href="{{ route('representative.pharmaceutical-products.index') }}" class="btn btn-secondary">
                <i class="ti ti-x"></i>
                {{ __('general.cancel') }}
            </a>
            <button type="submit" class="btn btn-primary">
                <i class="ti ti-check"></i>
                {{ __('products.register_product') }}
            </button>
        </div>
    </form>
</div>
