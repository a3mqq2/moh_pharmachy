            ...
            <div class="card-header">
                <div class="d-flex justify-content-between align-items-center">
                    <h5 class="mb-0">
                        ...
                        {{ __('users.edit_user_name', ['name' => $user->name]) }}
                    </h5>
                ...
                <form action="{{ route('admin.users.update', $user) }}" method="POST">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-3">
                            ...
                            @error('password')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                            <small class="text-muted">{{ __('users.password_leave_empty') }}</small>
                        </div>
                                ...
                                @foreach($departments->whereNull('parent_id') as $dept)
                                    <optgroup label="{{ $dept->name }}">
                                        ...
                                        @foreach($dept->children as $child)
                                            <option value="{{ $child->id }}" {{ old('department_id', $user->department_id) == $child->id ? 'selected' : '' }}>&nbsp;&nbsp;↳ {{ $child->name }}</option>
                                        @endforeach
                                    </optgroup>
                                @endforeach
                            </select>
                            ...
                            <div class="row g-2">
                                @foreach($permissions as $group => $groupPermissions)
                                    @php
                                        ...
                                        $checkedCount = collect($groupPermissions)->filter(fn($p) => in_array($p->name, old('permissions', $userPermissions)))->count();
                                                ...
                                                @foreach($groupPermissions as $permission)
                                                    <div class="d-flex align-items-center justify-content-between py-1 {{ !$loop->last ? 'border-bottom' : '' }}">
                                                        ...
                                                        <div class="form-check form-switch mb-0 ms-1">
                                                            <input class="form-check-input perm-checkbox perm-{{ $group }}" type="checkbox" name="permissions[]" value="{{ $permission->name }}" id="perm_{{ $permission->id }}" role="switch" style="width:1.8em;height:.9em;" {{ in_array($permission->name, old('permissions', $userPermissions)) ? 'checked' : '' }}>
                                                        </div>
                                                    </div>
                                                @endforeach
                                            </div>
                            ...
                            <div class="card bg-light border-0">
                                <div class="card-body">
                                    <h6 class="card-title text-muted mb-3">
                                        ...
                                        {{ __('users.additional_info') }}
                                    </h6>
                                    <div class="row">
                                        <div class="col-md-4">
                                            <small class="text-muted">{{ __('users.status_label') }}</small>
                                            <div>
                                                @if($user->is_active)
                                        ...
                                        <div class="col-md-4">
                                            <small class="text-muted">{{ __('users.created_at_label') }}</small>
                                            <div>{{ $user->created_at->format('Y/m/d H:i') }}</div>
                                        </div>
                                        <div class="col-md-4">
                                            <small class="text-muted">{{ __('users.last_updated_label') }}</small>
                    ...
                    <div class="d-flex justify-content-end gap-2">
                        ...
                        <button type="submit" class="btn btn-primary">
                            <i class="fas fa-save me-1"></i> {{ __('users.save_changes') }}
                        </button>
                    </div>
                </form>
            </div>
