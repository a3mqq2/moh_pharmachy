        ...
        <div class="card">
            <div class="card-header">
                <div class="d-flex align-items-center justify-content-between">
                    <h5 class="mb-0"><i class="ti ti-sitemap me-2"></i>الهيكل التنظيمي</h5>
                    <a href="{{ route('admin.departments.create') }}" class="btn btn-primary">
                        <i class="ti ti-plus me-1"></i>قسم جديد
                    </a>
                </div>
            </div>
            <div class="card-body">
                    ...
                    <div class="card-body">
                        <div class="d-flex align-items-center justify-content-between mb-2">
                            <div class="d-flex align-items-center gap-2">
                                ...
                                @endif
                                <span class="badge bg-light-primary">{{ $department->users->count() }} مستخدم</span>
                            </div>
                            <div class="d-flex gap-1">
                                <a href="{{ route('admin.departments.members', $department) }}" class="btn btn-sm btn-outline-info" title="الأعضاء">
                                    <i class="ti ti-users"></i>
                                </a>
                                <a href="{{ route('admin.departments.edit', $department) }}" class="btn btn-sm btn-outline-warning" title="تعديل">
                                    <i class="ti ti-edit"></i>
                                </a>
                                <form action="{{ route('admin.departments.toggle-status', $department) }}" method="POST" class="d-inline">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="btn btn-sm btn-outline-{{ $department->is_active ? 'secondary' : 'success' }}" title="{{ $department->is_active ? 'تعطيل' : 'تفعيل' }}">
                            ...
                            <div class="d-flex align-items-center justify-content-between py-2 border-bottom">
                                <div class="d-flex align-items-center gap-2">
                                    ...
                                    @if(!$child->is_active)
                                        <span class="badge bg-secondary">معطل</span>
                                    @endif
                                    <span class="badge bg-light-info">{{ $child->users->count() }} مستخدم</span>
                                    @if($child->description)
                                        <small class="text-muted">- {{ Str::limit($child->description, 40) }}</small>
                                    @endif
                                </div>
                                <div class="d-flex gap-1">
                                    <a href="{{ route('admin.departments.members', $child) }}" class="btn btn-sm btn-outline-info" title="الأعضاء">
                                        <i class="ti ti-users"></i>
                                    </a>
                                    <a href="{{ route('admin.departments.edit', $child) }}" class="btn btn-sm btn-outline-warning" title="تعديل">
                                        <i class="ti ti-edit"></i>
                                    </a>
                                    <form action="{{ route('admin.departments.toggle-status', $child) }}" method="POST" class="d-inline">
                                        @csrf
                                        @method('PATCH')
                                        <button type="submit" class="btn btn-sm btn-outline-{{ $child->is_active ? 'secondary' : 'success' }}" title="{{ $child->is_active ? 'تعطيل' : 'تفعيل' }}">
                ...
                <div class="text-center text-muted py-5">
                    ...
                    <a href="{{ route('admin.departments.create') }}" class="btn btn-primary btn-sm">
                        <i class="ti ti-plus me-1"></i>إنشاء أول قسم
                    </a>
                </div>
                @endforelse
            </div>
